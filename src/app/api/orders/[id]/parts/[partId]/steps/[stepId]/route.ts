...
export async function PUT(
  req: NextRequest,
  { params }: { params: { id: string; partId: string; stepId: string } }
) {
  ...
  const steps = await prisma.productionStep.findMany({ where: { partId: params.partId } });
  const step = steps.find((s) => s.id === params.stepId);
  if (!step) return NextResponse.json({ error: "Addım tapılmadı" }, { status: 404 });
...
export async function DELETE(
  ...
  if (!step || step.partId !== params.partId) {
    return NextResponse.json({ error: "Addım tapılmadı" }, { status: 404 });
  }
